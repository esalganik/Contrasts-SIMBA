...
xlsxDir = fullfile(scriptDir, 'export_excel');
...
% Heating GPS from NetCDF heating axes
latH30_raw = readVarSafe(ncFile, 'latitude_heating030_raw');
lonH30_raw = readVarSafe(ncFile, 'longitude_heating030_raw');
latH30 = readVarSafe(ncFile, 'latitude_heating030');
lonH30 = readVarSafe(ncFile, 'longitude_heating030');

latH120_raw = readVarSafe(ncFile, 'latitude_heating120_raw');
lonH120_raw = readVarSafe(ncFile, 'longitude_heating120_raw');
latH120 = readVarSafe(ncFile, 'latitude_heating120');
lonH120 = readVarSafe(ncFile, 'longitude_heating120');

% Optional fallback for old NetCDF files without heating GPS
if isempty(latH30) || isempty(lonH30)
    [latH30, lonH30, ~] = interpolateGeo(timeT, latT, lonT, geoFlagT, timeH30);
end
if isempty(latH30_raw) || isempty(lonH30_raw)
    [latH30_raw, lonH30_raw] = interpolateRaw(timeT, latT_raw, lonT_raw, timeH30);
end

if isempty(latH120) || isempty(lonH120)
    [latH120, lonH120, ~] = interpolateGeo(timeT, latT, lonT, geoFlagT, timeH120);
end
if isempty(latH120_raw) || isempty(lonH120_raw)
    [latH120_raw, lonH120_raw] = interpolateRaw(timeT, latT_raw, lonT_raw, timeH120);
end

geoFlagH30 = interpTimeSeries(timeT, geoFlagT, timeH30, 'nearest');
geoFlagH120 = interpTimeSeries(timeT, geoFlagT, timeH120, 'nearest');
...
baseRows = {
    ...
    'LATITUDE, raw GPS [deg]', 'deg', 'Raw GPS latitude on this data time axis';
    'LONGITUDE, raw GPS [deg]', 'deg', 'Raw GPS longitude on this data time axis';
    'LATITUDE, quality-controlled [deg]', 'deg', 'Quality-controlled latitude on this data time axis';
    'LONGITUDE, quality-controlled [deg]', 'deg', 'Quality-controlled longitude on this data time axis';
