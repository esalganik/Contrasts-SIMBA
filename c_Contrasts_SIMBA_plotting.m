...
ncDir = fullfile(scriptDir, 'export_netcdf');
...
cmapDir = fullfile(scriptDir, 'colormaps');
load(fullfile(cmapDir, 'vik.mat'));
load(fullfile(cmapDir, 'oslo.mat'));
...
for i = 1:numel(cfg)
    C = cfg(i);
    ncFile = fullfile(ncDir, sprintf('2025%s.nc', C.id));
...
exportgraphics(gcf, fullfile(figDir, "SIMBA_temperature_summary.png"), "Resolution", 300);

makeSummaryFigureFromNetCDF(proc, ...
    'gradient', gradLevels, gradCLim, vik, ylimsCommon, nTicks, c_reg);
exportgraphics(gcf, fullfile(figDir, "SIMBA_gradient_summary.png"), "Resolution", 300);

makeSummaryFigureFromNetCDF(proc, ...
    'heating', heatLevels, heatCLim, flipud(oslo), ylimsCommon, nTicks, c_reg);
exportgraphics(gcf, fullfile(figDir, "SIMBA_heating030_summary.png"), "Resolution", 300);

makeSummaryFigureFromNetCDF(proc, ...
    'heating120', heat120Levels, heat120CLim, flipud(oslo), ylimsCommon, nTicks, c_reg);
exportgraphics(gcf, fullfile(figDir, "SIMBA_heating120_summary.png"), "Resolution", 300);
