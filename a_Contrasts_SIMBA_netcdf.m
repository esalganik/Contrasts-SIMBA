...
pickDir = fullfile(scriptDir, 'data', 'Interfaces');
outDir = fullfile(scriptDir, 'export_netcdf');
...
cfg(1).doi = 'https://doi.pangaea.de/10.1594/PANGAEA.994096'; % 2025T143
...
cfg(2).doi = 'https://doi.pangaea.de/10.1594/PANGAEA.994100'; % 2025T144
...
cfg(3).doi = 'https://doi.pangaea.de/10.1594/PANGAEA.994105'; % 2025T145
...
cfg(4).doi = 'https://doi.pangaea.de/10.1594/PANGAEA.994087'; % 2025T135
...
cfg(5).doi = 'https://doi.pangaea.de/10.1594/PANGAEA.994092'; % 2025T136
...
[tH, latH_raw, lonH_raw, H] = cropSeries(tH, latH, lonH, H, C.crop_heat, 'HEAT030', C.id);
[tH120, latH120_raw, lonH120_raw, H120] = cropSeries(tH120, latH120, lonH120, H120, C.crop_heat120, 'HEAT120', C.id);
...
if ~isempty(H)
    ...
    H = H(goodH,:);
    tH = tH(goodH);
    latH_raw = latH_raw(goodH);
    lonH_raw = lonH_raw(goodH);
end
...
if ~isempty(H120)
    ...
    latH120_raw = latH120_raw(goodH120);
    lonH120_raw = lonH120_raw(goodH120);
end

latH = interpGeoToTime(t, latT, tH);
lonH = interpLonToTime(t, lonT, tH);

latH120 = interpGeoToTime(t, latT, tH120);
lonH120 = interpLonToTime(t, lonT, tH120);
    ...
    outFile = fullfile(outDir, sprintf('2025%s.nc', C.id));
    if isfile(outFile)
        delete(outFile);
    end
...
writeBuoyNetCDF(outFile, C, ...
    ...
    tH, latH_raw, lonH_raw, latH, lonH, H, ...
    tH120, latH120_raw, lonH120_raw, latH120, lonH120, H120, ...
    depth, ...
...
nccreate(outFile, 'latitude_heating030_raw', ...
    'Dimensions', {'time_heating030', nH}, 'Datatype', 'double');
ncwrite(outFile, 'latitude_heating030_raw', double(latH_raw(:)));
ncwriteatt(outFile, 'latitude_heating030_raw', 'standard_name', 'latitude');
ncwriteatt(outFile, 'latitude_heating030_raw', 'long_name', 'raw latitude from 30 s heating file');
ncwriteatt(outFile, 'latitude_heating030_raw', 'units', 'degrees_north');

nccreate(outFile, 'longitude_heating030_raw', ...
    'Dimensions', {'time_heating030', nH}, 'Datatype', 'double');
ncwrite(outFile, 'longitude_heating030_raw', double(lonH_raw(:)));
ncwriteatt(outFile, 'longitude_heating030_raw', 'standard_name', 'longitude');
ncwriteatt(outFile, 'longitude_heating030_raw', 'long_name', 'raw longitude from 30 s heating file');
ncwriteatt(outFile, 'longitude_heating030_raw', 'units', 'degrees_east');

nccreate(outFile, 'latitude_heating030', ...
    'Dimensions', {'time_heating030', nH}, 'Datatype', 'double');
ncwrite(outFile, 'latitude_heating030', double(latH(:)));
ncwriteatt(outFile, 'latitude_heating030', 'standard_name', 'latitude');
ncwriteatt(outFile, 'latitude_heating030', 'long_name', 'quality-controlled latitude interpolated from temperature GPS to 30 s heating time axis');
ncwriteatt(outFile, 'latitude_heating030', 'units', 'degrees_north');

nccreate(outFile, 'longitude_heating030', ...
    'Dimensions', {'time_heating030', nH}, 'Datatype', 'double');
ncwrite(outFile, 'longitude_heating030', double(lonH(:)));
ncwriteatt(outFile, 'longitude_heating030', 'standard_name', 'longitude');
ncwriteatt(outFile, 'longitude_heating030', 'long_name', 'quality-controlled longitude interpolated from temperature GPS to 30 s heating time axis');
ncwriteatt(outFile, 'longitude_heating030', 'units', 'degrees_east');
...
ncwriteatt(outFile, 'temperature_change_30s', ...
    'coordinates', 'time_heating030 depth latitude_heating030 longitude_heating030');
...
nccreate(outFile, 'latitude_heating120_raw', ...
    'Dimensions', {'time_heating120', nH120}, 'Datatype', 'double');
ncwrite(outFile, 'latitude_heating120_raw', double(latH120_raw(:)));
ncwriteatt(outFile, 'latitude_heating120_raw', 'standard_name', 'latitude');
ncwriteatt(outFile, 'latitude_heating120_raw', 'long_name', 'raw latitude from 120 s heating file');
ncwriteatt(outFile, 'latitude_heating120_raw', 'units', 'degrees_north');

nccreate(outFile, 'longitude_heating120_raw', ...
    'Dimensions', {'time_heating120', nH120}, 'Datatype', 'double');
ncwrite(outFile, 'longitude_heating120_raw', double(lonH120_raw(:)));
ncwriteatt(outFile, 'longitude_heating120_raw', 'standard_name', 'longitude');
ncwriteatt(outFile, 'longitude_heating120_raw', 'long_name', 'raw longitude from 120 s heating file');
ncwriteatt(outFile, 'longitude_heating120_raw', 'units', 'degrees_east');

nccreate(outFile, 'latitude_heating120', ...
    'Dimensions', {'time_heating120', nH120}, 'Datatype', 'double');
ncwrite(outFile, 'latitude_heating120', double(latH120(:)));
ncwriteatt(outFile, 'latitude_heating120', 'standard_name', 'latitude');
ncwriteatt(outFile, 'latitude_heating120', 'long_name', 'quality-controlled latitude interpolated from temperature GPS to 120 s heating time axis');
ncwriteatt(outFile, 'latitude_heating120', 'units', 'degrees_north');

nccreate(outFile, 'longitude_heating120', ...
    'Dimensions', {'time_heating120', nH120}, 'Datatype', 'double');
ncwrite(outFile, 'longitude_heating120', double(lonH120(:)));
ncwriteatt(outFile, 'longitude_heating120', 'standard_name', 'longitude');
ncwriteatt(outFile, 'longitude_heating120', 'long_name', 'quality-controlled longitude interpolated from temperature GPS to 120 s heating time axis');
ncwriteatt(outFile, 'longitude_heating120', 'units', 'degrees_east');
...
ncwriteatt(outFile, 'temperature_change_120s', ...
    'coordinates', 'time_heating120 depth latitude_heating120 longitude_heating120');
...
function yq = interpGeoToTime(tSource, ySource, tQuery)
...
xSource = posixtime(tSource(:));
xQuery = posixtime(tQuery(:));

ySource = double(ySource(:));
good = isfinite(xSource) & isfinite(ySource);

if nnz(good) < 2
    yq = nan(numel(tQuery),1);
    return
end

yq = interp1(xSource(good), ySource(good), xQuery, 'linear', 'extrap');
yq = yq(:);
end

function lonq = interpLonToTime(tSource, lonSource, tQuery)
...
lonSource = double(lonSource(:));
good = isfinite(lonSource);

lonUnwrapped = nan(size(lonSource));
lonUnwrapped(good) = rad2deg(unwrap(deg2rad(lonSource(good))));

lonq = interpGeoToTime(tSource, lonUnwrapped, tQuery);
lonq = wrapTo180Local(lonq);
end