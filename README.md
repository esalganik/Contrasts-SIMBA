...
The buoys recorded vertical temperature profiles within the snow and sea ice column, as well as temperature changes during active heating cycles (30 s and 120 s). The dataset includes time-resolved measurements of temperature and temperature change during active heating cycles on a fixed vertical grid referenced to the water level. Associated geolocation is provided for each observation timestamp.

The raw GPS data contain obvious errors. Therefore, both raw and corrected (quality-controlled) latitude and longitude are provided for each buoy. A geolocation flag indicates original, interpolated, invalid, edge-extrapolated, or long-gap-filled positions.
...
The dataset includes time and depth coordinates; geographic position (raw and quality-controlled) with flags; temperature and temperature change during active heating cycles; interface depths (air–snow, snow–ice, ice–water); derived snow depth and sea ice thickness; and manual interface measurements.
...
SIMBA buoy data were primarily obtained from the Meereisportal database (data.meereisportal.de/relaunch/buoy.php). For buoy 2025T135, data from 31 July to 7 August 2025 were manually added from files downloaded directly from the buoy, while the remaining data were obtained from Meereisportal.
